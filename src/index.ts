...
import { cancel, confirm, intro, isCancel, outro, progress, text } from '@clack/prompts'
...
;(async () => {
  ...
  const projectName = (await text({
    ...
    validate: value => {
      ...
      if (!result.validForNewPackages)
        return result.errors?.[0] || 'invalid npm package name, press backspace to clear'
    },
  })) as string
  ...
  if (npxOption) {
    await rename(`${destination}/package.npx.json`, `${destination}/package.json`)
  } else {
